...
def vggStyleNet(data_path = None, split='train', batch_size=128, im_shape=(96, 96),
    ...
    print inspect.getargspec(L.Dropout)
...
def normalStyleNet(data_path = None, split='train', batch_size=128, im_shape=(96, 96),
                 selection='all', random_flip=True, from_scratch=False):
    num_output = ST().getOutNum(selection)
    net = caffe.NetSpec()
    param_str = {'batch_size': batch_size, 'path': data_path,
    ...
    if split == 'deploy':
        ...
        net.data, net.label = L.Python(python_param=python_param, ntop=2)
    net.conv1, net.relu1 = conv_relu(net.data, nout=32, ks=3)
    net.pool1 = max_pooling(net.relu1, ks=2)
    net.conv2, net.relu2 = conv_relu(net.pool1, nout=64, ks=2)
    net.pool2 = max_pooling(net.relu2, ks=2)
    net.conv3, net.relu3 = conv_relu(net.pool2, nout=128, ks=2)
    ...
    if not split == 'deploy':
        net.loss = L.EuclideanLoss(net.fc6, net.label)

    file_name = osp.join(model_dir, split + '_n_' + selection + '.prototxt')
    with open(file_name, 'w') as f:
        f.write(str(net.to_proto()))
        f.close()
        return f.name
